...
//Single Responsibility Principle (SRP): The class is responsible for handling actions related to the user dashboard, such as filtering bus data, displaying the dashboard, and booking buses.
//SOLID Pinciples Violated:
//Open/Closed Principle (OCP): Similar to AdminController, this class could be closed for modification by extracting the logic for retrieving the current user's name into a separate method or component.
//Dependency Inversion Principle (DIP): The class directly depends on concrete implementations of UserRepository and BusDataRepository, violating the DIP. It would be better to depend on abstractions (interfaces) instead.