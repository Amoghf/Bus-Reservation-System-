...
//Single Responsibility Principle (SRP): The class handles administrative actions like saving bus data, displaying all records, and deleting records.
//SOLID Principles Violated:
//Open/Closed Principle (OCP): The class could be closed for modification by extracting the logic for retrieving the current user's name into a separate method or component. This would make it easier to extend the behavior without modifying existing code.
//Dependency Inversion Principle (DIP): The class directly depends on concrete implementations of UserRepository and BusDataRepository. It would be better to depend on abstractions (interfaces) instead of concrete classes to facilitate easier testing and future changes.